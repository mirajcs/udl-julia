...
# ╔═╡ f5608b2a-c1a4-41d8-aaa0-04c56dbec971
using Plots

# ╔═╡ 3de5c28c-8600-11f1-9f73-f3007f12c46c
md"# Notebook 3.4 -- Activation Functions

The purpose of this practical is to experimnet with different activation functions"

# ╔═╡ bbd309e5-d706-4346-a6e0-bae69b816dcc
md"Plot the shallow neural network. We'll assume that input is in range [0,1] and output is in [-1,1]. 
If the PlotAll flag is set to true, then we'll plot all the intermediate stages as in figure 3.3"

# ╔═╡ 69b27ef4-4ddb-43c3-b15e-9acd85d185fc
function PlotNeural(x, y, Pre₁, Pre₂, Pre₃, Act₁, Act₂, Act₃, WAct₁, WAct₂, WAct₃; PlotAll = false, xData = nothing, yData = nothing)
    # Output plot.
    out = plot(x, y, xlabel="Input, x", ylabel="Output, y",
               xlim=(0,1), ylim=(-1,1), aspect_ratio=0.5, legend=false)
    if xData !== nothing
        scatter!(out, xData, yData, color=:magenta, markershape=:circle)
    end
    PlotAll || return out
    ...
    p₃₁ = plot(x, WAct₁, color=:red,   ylabel="Weighted Act", legend=false)
    p₃₂ = plot(x, WAct₂, color=:blue,  ylabel="Weighted Act", legend=false)
    p₃₃ = plot(x, WAct₃, color=:green, ylabel="Weighted Act", legend=false)

    for p in (p₁₁, p₁₂, p₁₃, p₂₁, p₂₂, p₂₃, p₃₁, p₃₂, p₃₃)
        plot!(p, xlim=(0,1), ylim=(-1,1))
    end

    return plot(p₁₁, p₁₂, p₁₃, p₂₁, p₂₂, p₂₃, p₃₁, p₃₂, p₃₃, out,
                layout=@layout([a b c; d e f; g h i; _ j _]), size=(900, 900))
end


# ╔═╡ d0943e10-3956-49c0-8c0f-71265a41abcb
md"Define a shallow neural network with one input, and three hidden units"

# ╔═╡ 55835ed3-cf6f-4cf2-8f12-e1268d58b2d9
...
# ╔═╡ ae2db1ad-fb06-4918-b84d-32c1189a7b6b
md"Define the Rectified Linear Unit (ReLU) function" 

# ╔═╡ 95fc431b-e34a-4705-9606-7e03701c38ba
function ReLU(preactivation)
    activation = max.(0.0, preactivation)
    return activation 
end

# ╔═╡ c501f535-574b-48f0-9127-646a55ef8119
md"### Let's run the network with a ReLU function."

# ╔═╡ ce8bd837-bcd8-455d-933b-7d94379ebbd2
begin
	θ₁₀ = 0.3; θ₁₁ = -1.0 
	θ₂₀ = -1.0; θ₂₁ = 2.0 
	θ₃₀ = -0.5; θ₃₁ = 0.65 
	ϕ₀ = 0.3; ϕ₁ = 0.5; ϕ₂ = -1.0; ϕ₃ = 7.0 
...
# ╔═╡ bc3c4192-803a-49af-b100-70f7542982ee
md"# Sigmoid Activation Function 

The ReLU isn't only kind of activation function. For a long time, people used sigmoid functions. A logistic sigmoid function is defined by the equation 
```math 
f(z) = \dfrac{1}{1 + exp[-10z]}
``` 
(Note that the factor 10 is not standard, but it allows to plot on the same axes as the ReLU examples)"

# ╔═╡ 5b9f8779-30bb-4df9-a77e-b41a943a4434
md"Define the Sigmoid function"

# ╔═╡ 5e515e13-307c-47c2-8921-d9c44c5a60cc
function sigmoid(preactivation)
    activation = 1.0 ./ (1.0 .+ exp.(-10 * preactivation))
    return activation 
end

# ╔═╡ 263910f9-2315-4481-b8d3-12654dd08059
begin
	z = -1:0.01:0.99 
	sigZ = sigmoid(z) 
...
# ╔═╡ af7c3812-4817-464e-89e1-992fd0525c0c
md"   Let's see what happens when we use this activation function in a neural network "

# ╔═╡ 9865ffa5-780a-48ef-ae18-1a5d69dc25b4
begin
	θ₁₀¹ = 0.3; θ₁₁¹ = -1.0 
	θ₂₀¹ = -1.0; θ₂₁¹ = 2.0 
	θ₃₀¹ = -0.5; θ₃₁¹ = 0.65 
...
# ╔═╡ 480f1714-5347-49a2-93df-ecb53a7b9ed3
md"# Heaviside Activation Function 

The Heaviside activation function defined as
```math
...
# ╔═╡ a93ce661-23fc-40bb-a679-c6dfbaafba56
function heaviside(preactivation)
    activation = Float64.(preactivation .>= 0.0)
    return activation 
end

# ╔═╡ a21159ad-f885-42e0-876e-9bb7db1d1d11
begin
	z² = -1:0.01:0.99 
	hev_Z = heaviside(z²) 

	plot(z, hev_Z, color=:red, xlim=(-1,1), ylim=(-1,2), xlabel="z", ylabel="sig[z]", legend=false)
end

# ╔═╡ 8b86f521-e47f-488a-97b5-60f0832fad09
begin
	θ₁₀² = 0.3; θ₁₁² = -1.0 
	θ₂₀² = -1.0; θ₂₁² = 2.0 
	θ₃₀² = -0.5; θ₃₁² = 0.65 
...
# ╔═╡ 6f08dfd0-0e3b-4b28-bdcf-561d95cb4360
md"# Linear Activation Function 
...
# ╔═╡ e25d42d6-5318-467d-b20c-00ade8b3b3ad
...
# ╔═╡ 55797d6c-0843-4fbe-bc91-89092b544048
md"## TODO
...
# ╔═╡ f770e10f-15b3-4d12-89fc-3cd714239140
begin
	θ₁₀³ = 0.3; θ₁₁³ = -1.0 
	θ₂₀³ = -1.0; θ₂₁³ = 2.0 
	θ₃₀³ = -0.5; θ₃₁³ = 0.65 
