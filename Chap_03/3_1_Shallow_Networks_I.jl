...
# ╔═╡ 379dd980-3c28-4553-ae11-52f41d604ef1
using Plots

# ╔═╡ c600d639-b26c-4439-aded-0d0fdb4e6e1e
using Printf

# ╔═╡ fd7a196a-85db-11f1-bac4-c1c72d2bd064
md"# **Notebook 3.1 -- Shallow Neural Networks I**
...
# ╔═╡ 7d791225-91bc-4d18-9231-744711083800
md"Let's first construct the shallow neural network with one input, three hidden units, and one output described in section 3.1 of the book."

# ╔═╡ 7b59b415-3411-4db0-9523-c402e1d811b9
md"Define the Rectified Linear Unit (ReLU) function"

# ╔═╡ cd04b2ca-0ddd-466b-89a8-4add3c51b8d6
function ReLU(preactivation)
    activation = max.(0.0, preactivation)
    return activation 
end

# ╔═╡ 521dff3e-2908-402d-a6ca-4dc99bf5a582
z = collect(-5:0.1:5)

# ╔═╡ ff17864f-d4a9-4024-9ec2-609a3fca2078
RELU_z = ReLU(z); nothing

# ╔═╡ 56f92a62-459c-4974-9e2e-f7aa0d0b730d
md"plot the ReLU function"

# ╔═╡ c1682021-c813-4fa9-8eda-06a72bf44801
...
# ╔═╡ 7b14cebf-7a75-456b-9c0f-7fecb674e20e
md"Define a shallow neural network with one input, one output, and three hidden units"

# ╔═╡ c0e69062-eed9-4b75-8696-9aa05c223346
...
# ╔═╡ 005d59f5-ca59-456f-8929-2841f0ffd505
function plot_neural(x, output, pre_1, pre_2, pre_3, act_1, act_2, act_3, w_act_1, w_act_2, w_act_3;
    ...
    # Just the output plot (Pluto renders a cell's RETURN value, not display()).
    ...
    # 3x3 grid of intermediates, with the output as a 4th-row cell (same size).
    # The two `_` leave the rest of the last row blank.
    l = @layout [a b c; d e f; g h i; _ j _]
    p = plot(layout=l, size=(900, 900), legend=false)
    ...
    # Output goes in the 10th panel (the single cell in the last row).
    plot!(p[10], x, output, xlims=(0,1), ylims=(-1,1),
          xlabel="Input, x", ylabel="Output, y")
    if x_data !== nothing
        scatter!(p[10], x_data, y_data, color=:purple)
    end

    return p
end

# ╔═╡ f2bc9c00-e818-4691-ac5b-d4489f883854
md"Define some parameters for neural network"

# ╔═╡ 11407d4e-b7dd-4e96-ad7b-47030e9e6436
begin
	θ₁₀ = 0.3; θ₁₁ = -1.0
	θ₂₀ = -1.0; θ₂₁ = 2.0
	θ₃₀ = -0.5; θ₃₁ = 0.65 
...
# ╔═╡ 85196590-009e-4486-9317-0ad250f1d0d5
md"Define a range input values "

# ╔═╡ 9528a790-1004-44f3-8e3d-079a7b51aec4
x = collect(0:0.01:1)

# ╔═╡ 1196e123-2f5b-4411-812e-7d8453c1bf18
md"run the neural network for each of these input values "

# ╔═╡ ee7b3d88-313b-4da0-bd80-b73c02f7fcfd
output, pre_1, pre_2, pre_3, act_1, act_2, act_3, w_act_1, w_act_2, w_act_3 = 
    shallow_1_1_3(x, ReLU, ϕ₀, ϕ₁, ϕ₂, ϕ₃, θ₁₀, θ₁₁, θ₂₀, θ₂₁, θ₃₀, θ₃₁)

# ╔═╡ 7a09e3da-26ab-4c0b-b555-9c765bacdfda
md"Then plot it"

# ╔═╡ e3ed9da5-b2bd-4895-ab31-0afb7905ecce
plot_neural(x, output, pre_1, pre_2, pre_3, act_1, act_2, act_3, w_act_1, w_act_2, w_act_3, plot_all=true)

# ╔═╡ 8a19770b-45c3-4db8-af8a-1c919c8eaa9f
md"# TODO
...
5. Change the parameters so that there are only two ''joints'' (including outside the range of the plot). There are actually three ways to do this. See if you can figure them all out.
6. With the original parameters, the second line segment is flat (i.e. has slope zero). How could you change theta_10 so that all of the segments have non-zero slopes?
7. What do you predict would happen if you multiply $\theta_{20}$ and $\theta_{21}$ by 0.5, and $\phi_2$ by 2.0? Check if you are correct.
8. What do you predict would happen if you multiply $\theta_{20}$ and $\theta_{21}$ by -0.5, and $\phi_2$ by -2.0? Check if you are correct."

# ╔═╡ 7e2abf08-c122-4346-b2eb-1acd08539359
begin
	θ₁₀¹ = 0.3; θ₁₁¹ = -1.0
	θ₂₀¹ = -1.0; θ₂₁¹ = 2.0
	θ₃₀¹ = -0.5; θ₃₁¹ = 0.65 
...
# ╔═╡ d9e1b56c-4ce8-4c69-8a5a-241db5e8a693
md"Define a range input values"

# ╔═╡ 115651fc-f100-4ba0-a47b-25585e437e77
x¹ = collect(0:0.01:1)

# ╔═╡ 9661fa42-18cb-46eb-bb33-7b39d566b9b0
md"run the neural network for each of these input values"

# ╔═╡ 0809d244-2ff3-47c5-952f-33e34396fa81
output¹, pre_1¹, pre_2¹, pre_3¹, act_1¹, act_2¹, act_3¹, w_act_1¹, w_act_2¹, w_act_3¹ = 
    shallow_1_1_3(x¹, ReLU, ϕ₀¹, ϕ₁¹, ϕ₂¹, ϕ₃¹, θ₁₀¹, θ₁₁¹, θ₂₀¹, θ₂₁¹, θ₃₀¹, θ₃₁¹)

# ╔═╡ 1602e09f-9d5d-4ae4-9539-e5ff84748c4e
md"Then plot it "

# ╔═╡ 45d2b18d-ac59-4ee2-86ad-54b82035fe87
plot_neural(x¹, output¹, pre_1¹, pre_2¹, pre_3¹, act_1¹, act_2¹, act_3¹, w_act_1¹, w_act_2¹, w_act_3¹, plot_all=true)

# ╔═╡ 31dd804c-d63d-45b8-9776-0c0118d502f8
md"# Least squares loss
...
# ╔═╡ 5b6f5b0a-5162-4c15-ac5c-b69b8ddf7038
md"Loss function"

# ╔═╡ 1adbff20-f399-4ff2-9064-e337f80d62cd
function least_square_loss(y_train, y_predict)
    loss = sum((y_predict .- y_train).^2)
    return loss
end 

# ╔═╡ e6ad7044-980a-409b-a5d5-f83cce2fd896
md"Define some parameters, run the neural network, and compute the loss "

# ╔═╡ 8ab2f505-1fe6-4a29-ad64-330b73cf0255
begin
	θ₁₀² = 0.3; θ₁₁² = -1.0
	θ₂₀² = -1.0; θ₂₁² = 2.0
	θ₃₀² = -0.5; θ₃₁² = 0.65 
...
# ╔═╡ ffe2937b-d69d-4d20-8730-4c1e7b038271
md"Define a range of input values "

# ╔═╡ 1b7f569b-88e7-4b6d-b321-9aeac0772a7e
x² = collect(0.0:0.01:0.99)

# ╔═╡ 89996f58-018e-447f-9869-21aa78361d8e
x_train² = [0.09291784, 0.46809093, 0.93089486, 0.67612654, 0.73441752, 0.86847339,
           0.49873225, 0.51083168, 0.18343972, 0.99380898, 0.27840809, 0.38028817,
           0.12055708, 0.56715537, 0.92005746, 0.77072270, 0.85278176, 0.05315950,
           0.87168699, 0.58858043]

# ╔═╡ eefc1924-bd24-4e6a-a49c-170cb86dda8d
...
# ╔═╡ 99a71c6b-15d8-48d3-bfb0-7a86a241e4f7
md"Run the neural network for each of these input values"

# ╔═╡ 12577835-ad7c-4fec-9c2d-61c1d13745d9
y², pre_1², pre_2², pre_3², act_1², act_2², act_3², w_act_1², w_act_2², w_act_3² =
    shallow_1_1_3(x², ReLU, ϕ₀², ϕ₁², ϕ₂², ϕ₃², θ₁₀², θ₁₁², θ₂₀², θ₂₁², θ₃₀², θ₃₁²)

# ╔═╡ 37c6ad33-b136-4712-b28b-e8f7527929f2
md"Plot with training data ovaerlaid"

# ╔═╡ da9d33f5-60a9-498d-8fe7-54fbcfdfa1ad
