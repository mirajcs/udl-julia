...
# ╔═╡ 27999c4c-53b1-47cc-88af-f64ace281563
using PlutoUI

# ╔═╡ 19ab311e-8354-46d5-b2b8-30c28d777a6f
md"# Notebook 5.3 - Multiclass Cross-Entropy Loss

This notebook investigate the multi-class cross-entropy loss. It follows from applying the formula in section 5.2 to a loss function based on the Categorical distribution."
...
md"Define a shallow neural network"
...
# ╔═╡ de38b94f-e628-4e40-b836-10dc2174795a
md"Get Parameters for model"

# ╔═╡ 22f65192-3aa8-47c5-99a0-7990dc008802
function GetParameters()
	β₀ = [0.3; -1.0; -0.5]
	Ω₀ = [-1.0; 1.8; 0.65]
	β₁ = [2.0; -2.0; 0.0]
	Ω₁ = [-24.0 -8.0 50.0; -2.0 8.0 -30.0; 16.0 -8.0 -8.0]
	return β₀, Ω₀, β₁, Ω₁
end 

# ╔═╡ 8d135603-bad4-445c-8706-20c3f644c91d
...
function PlotMultiClassClassification(xIn::AbstractVector, yOut, λ;
                                      xData = nothing, yData = nothing, title = nothing, size = (1200, 600))
    ...
    lines!(ax1, xIn, yOut[1, :], color = :red)
    lines!(ax1, xIn, yOut[2, :], color = :green)
    lines!(ax1, xIn, yOut[3, :], color = :blue)
    xlims!(ax1, 0, 1)
    ylims!(ax1, -4, 4)

    # Right panel : λ / probabilities
    ax2 = Axis(fig[1, 2];
               xlabel = L"Input, $x$",
               ylabel = L"$\lambda$ or Pr(y=k|x)",
               title = title === nothing ? "" : title)
    lines!(ax2, xIn, λ[1, :], color = :red)
    lines!(ax2, xIn, λ[2, :], color = :green)
    lines!(ax2, xIn, λ[3, :], color = :blue)
    xlims!(ax2, 0, 1)
    ylims!(ax2, -0.1, 1.05)

    # Overlay data points colored by class
    if xData !== nothing
        for i in 1:length(xData)
            if yData[i] == 1
                scatter!(ax2, [xData[i]], [-0.05], color = :red)
            elseif yData[i] == 2
                scatter!(ax2, [xData[i]], [-0.05], color = :green)
            elseif yData[i] == 3
                scatter!(ax2, [xData[i]], [-0.05], color = :blue)
            end
        end
    end
...
md"# Multiclass Classification

For multiclass classifications, the network must predict the probability of $K$ classes, using $K$ outputs. However, these probability must be non-negative and sum to one, and the network outputs can take arbitrary values. Hence, we pass the outputs through a softmax function which maps $K$ arbitrarry values to $K$ non-negative values that sum to one."

# ╔═╡ cc929350-c2e3-4620-b5b3-1ce83f594eb8
function  Softmax(ModelOut)
...
begin
	...
	λ = Softmax(ModelOut)
	PlotMultiClassClassification(xModel, ModelOut, λ; xData = xTrain, yData=yTrain, size=(1200, 600))
end
