...
This notebook runs a simple neural network on the MINIST 1D dataset as in figure 8.2a. It uses code from [https://github.com/greydanus/mnist1d](https://github.com/greydanus/mnist1d) to generate the data. "

# ╔═╡ 351640c0-0885-4869-bc48-1c59a0df42f0
begin
	url = "https://github.com/greydanus/mnist1d/raw/master/mnist1d_data.pkl"
	path = joinpath(@__DIR__, "mnist1d_data.pkl")
	isfile(path) || Downloads.download(url, path)

	data = Pickle.npyload(path)
end 

# ╔═╡ 9f22f652-f197-4045-9206-67d14fa66245
md"One sample per digit"

# ╔═╡ d16f1f43-44df-4cb8-973c-5a9379859f21
let 
	x, y = data["x"], data["y"]
...
let
	tx = data["templates"]["x"]   # 10 × 12, one row per digit
	fig = Figure(size=(900, 400))
	for d in 0:9
		ax = Axis(fig[d ÷ 5 + 1, d % 5 + 1]; title="$d", yreversed=true)
		lines!(ax, tx[d+1, :], 1:12; linewidth=2)
	end
	fig
end


# ╔═╡ e85eeba7-6064-48f9-a23f-c528af1d8be8
md"""
Examples in training set: $(length(data["y"]))

Examples in test set: $(length(data["y_test"]))

Length of each example: $(size(data["x"], 2))
"""
...
md"Two hidden layers of size 100 with ReLU activations, all weights He (Kaiming) initialised, bias zero."
